...
let handler = async (m, { conn }) => {
  ...
  try {
    let repo_Data = await axios.get('https://api.github.com/repos/PAPAIGWE241/QUEEN_SOFIA');
    let { stargazers_count, forks, owner, created_at, html_url } = repo_Data.data;

    let infobt = `
💋𝙸𝙼 𝚀𝚄𝙴𝙴𝙽 𝚂𝙾𝙵𝙸𝙰 𝙰 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝙱𝙾𝚃 𝙲𝙷𝚄𝙳𝙳𝚈 𝙱𝚄𝙳𝙳𝚈 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈 𝙿𝙰𝙿𝙰 𝙸𝙶𝚆𝙴 𝙰𝙽𝙳 𝙸𝙶𝚆𝙴 𝚃𝙴𝙲𝙷 𝙸𝙽𝙲𝙾𝚁𝙿𝙾𝚁𝙰𝚃𝙴💋

*❲❒❳ 𝚂𝚃𝙰𝚁𝚃: ${stargazers_count} Stars ⭐
*❲❒❳ Forks:* ${forks} Forks 🍴
*❲❒❳ Author:* *Salman Ahmad* 😎
*❲❒❳ Create:* ${created_at} 
*❲❒❳ Repo:* ${html_url}

🔰 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈 𝙿𝙰𝙿𝙰 𝙸𝙶𝚆𝙴🔰
